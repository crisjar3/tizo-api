...
INSERT INTO operator_account (id, display_name, email, role, active, created_at) VALUES
    ...
    ('op-002', 'Bruno Soporte', 'bruno.ops@tizo.local', 'OPERATOR', TRUE, '2026-01-01T00:00:00Z'),
    ('op-inactive', 'Operador Inactivo', 'inactive@tizo.local', 'OPERATOR', FALSE, '2026-01-01T00:00:00Z')
ON CONFLICT (id) DO UPDATE SET
...
ON CONFLICT (product_id, display_order) DO UPDATE SET
    url = EXCLUDED.url,
